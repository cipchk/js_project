    ...
    if (active(appinfo.bid, 4)) {
      ...
      switch (UI) {
        case "com.yyhd.joke.streamapp.main.MainActivity":
          log("主界面");
          if (regok_key && jsclick("id", "tv_praise")) {
            Idfa("注册成功");
            return true;
        ...
        case "com.yyhd.joke.login.login.view.LoginActivity":
          log("登录界面");
          if (jsclick("id", "checkbox", true, 2)) {
          ...
          } else if (jsclick("id", "verificationCodeView")) {
            if (sms_key) {
              sms_num = getMessage();
              if (sms_num) {
                sms_num_key = 0;
                for (var i = 0; i < sms_num.length; i++) {
                  sleep(1000);
                  setText(i, sms_num[i]);
                }
                sms_key = false;
...
function yuedu() {
  ...
  while (new Date().getTime() - timeLine < 3 * 60 * 1000) {
    ...
    log(timeLine - new Date().getTime());
    if (active(appinfo.bid, 4)) {
      if (jsclick("id", "searchIcon", true, 2)) {
      }
      var UI = currentActivity();
      log("UI", UI);
      switch (UI) {
        case "com.android.packgeinstaller.permission.ui.GranPermissionsMainActivity":
          log("主页面");
          if (jsclick("id", "searchIcon", true, 2)) {
          }
          break;
        case "com.hunantv.imgo.activity.MainActivity":
          log("主页面");
          if (jsclick("id", "searchIcon", true, 2)) {
          }
          break;
        case "com.mgtv.ui.search.SearchActivity":
          log("搜索界面");
          if (jsclick("text", "我的哈士奇男友")) {
            if (jsclick("text", "播放")) {
              jsclick("text", "2", true, 2);
            }
            // jsclick("text", "...", true, 2);
          } else if (jsclick("id", "ivClear", true, 2)) {
          } else {
            setText("我的哈士奇男友");
            sleep(3000);
            click(671, 1238);
          ...
          break;
        case "com.mgtv.ui.search.transfer.SearchTransferActivity":
          log("选集界面");
          // one_ = (59, 286);
          // _x = 117 * random(0, 5);
          // _y = 159 * random(0, 3);
          // if (jsclick("text", "选集")) {
          //   click(one_[1] + _x, one_[2] + _y);
          // }
          break;
        // case "com.mgtv.ui.player.VodPlayerPageActivity":
        //   log("播放界面");
        //   if (jsclick("id", "ivGuideOk", true, 2)) {
        //   }
        //   if (jsclick("id", "rlLikeIcon", true, 2)) {
        //     sleep(8000);
        //     var d = id("tvVideoIndex").selected(true).findOne(200);
        //     if (d) {
        //       log(d.text());
        //       if (ms({ text: Number(d.text()) + 1 }, true, 2)) {
        //       } else {
        //         return true;
        //       }
        //     }
...
var appinfo = {};
appinfo.name = "芒果tv";
appinfo.bid = "com.hunantv.imgo.activity";
...
function get_task() {
  ...
  postArr = {};
  postArr["phonename"] = "anzhuo";
  postArr["imei"] = "hemayun";
