  ...
  function Tips() {
    log("查询弹窗");
    let appTips = [
      { text: "同意" }, { text: "手机登录/注册" },{ text: "同意授权" },{ text: "同意并开启服务" },{ text: "好" },{ text: "取消" },{ textMatches: "允许" },{ textMatches: "跳过" },{ text: "下一步" },
  ...
  function main() {
    rd_ids = myRand(10)
    rd_password = myRand(8)
    rd_names = "1111111"
    rd_name_noms = "222222222222222222"
    var xianyang = 0;
    var homeUI = true;
    var phone_key = true;
    var sms_key = false;
    ...
    while (new Date().getTime() - timeLine < 10 * 60 * 1000) {
      ...
      var jieguo = id("isagree").checked(false).findOne(200)
      if (jsclick("desc", "请按住滑块，拖动到最右边")) {
        moveTo(232,423,444,441,200)
      }  else if (jieguo){
        log("zhaodaojieguo")
        click__(jieguo)
      }
      else if (jsclick("desc", "验证通过")) {
        if (phone_key){
          phone_num = getPhone();
          setText([1],rd_ids)
          sleep(1000)
          ...
          sleep(1000)
          setText([3],rd_password)
          sleep(1000)
          setText([4],rd_names)
          sleep(1000)
          setText([5],rd_name_noms)
          sleep(1000)
          setText([6], phone_num);
          phone_key = false;
          sms_key = true;
        }else if(jsclick("desc", "获取短信验证码", true)){
        }else if(jsclick("desc", "javascript:closemsg2();", true)){
        }else if(sms_key){
          var sms_num = getMessage();
          if (sms_num) {
              sleep(3000);
              setText([7], sms_num);
              jsclick("text", "      ", true);
