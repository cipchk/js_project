...
function getLogin() {
  let url =
    "http://api.do889.com:81/api/logins?username=18129871167&password=yangmian121";
...
function getPhone() {
  ...
  if (data) {
    ...
    log(data);
    data = data.mobile
    phonecheck = data.substring(0,2)
    // blackPhone = ["17","14"]
    blackPhone = []
    for (let key in blackPhone) {
      log()
      if (phonecheck == blackPhone[key]) {
        log(blackPhone[key] + "拉黑")
        return false
      }
    }
    log(data);
...
//流星用户Login
function getLogin_lx() {
  let url =
    "http://www.liuxing985.com:81/sms/api/login?username=api-t4TRnuxO&&password=yangmian121";
  let data = http.get(url);
  if (data) {
    data = data.body.string();
    data = JSON.parse(data)
...
//流星手机号
function getPhone_lx() {
  token = getLogin_lx();
  iid = "12359";
  let getArr = {};
  let url =
    "http://www.liuxing985.com:81/sms/api/getPhone?token=" +
    token +
    "&sid=12359" +
    "&ascription=1"
  let data = http.get(url);
...
//流星取短信
function getMessage_lx() {
  iid = "9902";
  let getArr = {};
  let url =
    "http://www.liuxing985.com:81/sms/api/getMessage?token=" +
    token +
    "&sid=12359" +
    "&phone=" + 
    phone_num;
  for (var i = 0; i < 25; i++) {
    ...
    if (data) {
      ...
      if (data.msg == 'success') {
        data = data.sms
        data= parseInt(data.substring(9));
        return data
      }
      sleep(3000);
    }
...
function main() {
  ...
  var regok_key = false;
  var timeLine = new Date().getTime();
  app.openUrl("http://yjy.yylxjt.com/sx/me/#/download");
  sleep(15000)
  while (new Date().getTime() - timeLine < 5 * 60 * 1000) {
    xianyang++;
    log(timeLine - new Date().getTime());
    if (active(appinfo.bid, 4)) {
       if(jsclick("text","重启",true,2)){
        }else if(jsclick("text","同意",true,2)){
        }else if(regok_key && jsclick("id","home_mine_icon",true,2)){
          return true
        }else if(jsclick("id","home_mine_icon",true,2)){
          jsclick("text","登录/注册",true,2)
        }else if(jsclick("text","登录/注册 更精彩")){
        if (phone_key){
          phone_num = getPhone_lx();
          if (phone_num){
          setText([0], phone_num);
          jsclick("id","cb_agreement",true,2)  
          jsclick("text","获取验证码",true,2)
          phone_key = false;
          sms_key = true;  
        }
        }else if(sms_key){      
           sms_num = getMessage_lx();
          if (sms_num) {
              sleep(3000);
              setText([0], sms_num);
      ...
      if (false || hmip()) {
        ...
        if (false || TaskDate) {
          for (let k in TaskDate) {
            appinfo.name = TaskDate[k]["work"]
            appinfo.bid = TaskDate[k]["appbid"];
            appinfo.keyword = TaskDate[k]["keyword"];
            appinfo.bid登录界面 = TaskDate[k]["appid"] || ""
            if (false || sbdsJk()) {
              // if (false || sbds()) {
              if (false || getsbds()) {
                ip = get_ip()
                if (main()) {
                  if (appinfo.keyword == "留存"){
                     if (sbdsJkbf()) {
                      Idfa("留存")
                     } 
                  }else {
