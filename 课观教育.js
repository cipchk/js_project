...
function nmbc(nmbk) {
  ...
  const nmbkey_list = {
    0: [360, 1219],1: [196, 883],2: [363, 883],3: [523, 883], 4: [196, 993], 5: [363, 993], 6: [523, 993],7: [196, 1106], 8: [363, 1106],9: [523, 1106],
  };
...
function Tips() {
  log("查询弹窗");
  let appTips = [
    { text: "同意" }, { text: "手机登录/注册" },{ text: "同意授权" },{ text: "同意并继续" },{ text: "好" },{ text: "取消" },{ textMatches: "允许" },{ textMatches: "跳过" },{ text: "下一步" },
    { text: "确定" },{ textMatches: "忽略" },{ textMatches: "确认" },{ textMatches: ".*关闭应用.*" },{ text: "稍后再说" },{ textMatches: ".*重新打开应用.*" },{ text: "重新加载" },
  ];
...
function main() {
  ...
  while (new Date().getTime() - timeLine < 10 * 60 * 1000) {
    ...
    if (jsclick("text", "未来的人民教师\n欢迎您")) {
      jsclick("text", "登录", true);
    } else if (jsclick("text", "验证码登录")) {
      if (phone_key){
        phone_num = hb_getPhone();
        setText([0], phone_num);
        jsclick("text", "获取验证码", true);
        phone_key = false;
        sms_key = true;
      }else if(sms_key){
    ...
    } else if (jsclick("text", "选择考试类型")){ 
      click((30+5+(369*random(0,1))),(244+5+(114*random(0,1))))
      sleep(1000)
      click((30+5+(172*random(0,3))),(628+5))
      sleep(1000)
      click((30+5+(172*random(0,3))),(898+5+(114*random(0,2))))
    }else if (jsclick("text", "选择报考地区"))  {
      click((30+5+(170*random(0,3))),(327+5+(130*random(0,6))))
    }
    else if (jsclick("text", "选择报考城市")){
      click((30+5+(170*random(0,3))),(548+5+(160*random(0,3))))
    }
    else if (jsclick("text", "以后再学",true,2)){}
    else if (jsclick("id", "iv_close",true,2)){
    }
    else if (regok_key && jsclick("text","课程") && jsclick("text","练习")
    ) {
      return true;
    }else if (jsclick("text", "我的")){
      click(606,1233)
    }
    else if (active(appinfo.bid, 4)) {}
    Tips();
    sleep(1000);
  }
}
...
var appinfo = {};
appinfo.name = "课观教育";
device_num = "anzhuohb001";
phone_imei = device_num
phone_name = device_num
appinfo.bid = "com.ekeguan.jiaoshipai";
