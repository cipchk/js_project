...
function Idfa() {
  ...
  postArr["idfa"] = appinfo.imei;
  postArr["password"] = appinfo.device;
  postArr["account"] = appinfo.brand;
    ...
    if (active(appinfo.bid, 4)) {
      ...
      switch (UI) {
        case "com.soyoung.module_main.ui.MainActivity":
          ...
          if (mainokKEY) {
            return true;
          } else if (jsclick("id", "imgClose", true, 2)) {
          } else if (jsclick("text", click_[rd(0, 10)], true, 15)) {
            sleep(6000);
            if (jsclick("id", "imgClose", true, 2)) {
            }
            // for (var i = 0; i < 12; i++) {
