...
function sbdsJk() {
  ...
  var result = shell("pm clear com.sf.activity", true);
  log(result);
  if (result.code == 0) {
    toast("一键清理 执行成功");
  } else {
...
function main() {
  ...
  while (new Date().getTime() - timeLine < 1 * 60 * 1000) {
    ...
    if (active(appinfo.bid, 4)) {
      ...
      log("UI", UI);
      switch (UI) {
        case "com.sf.activity.MainActivity":
          log("主界面");
          if (jsclick("text", "开始体验", true, 2)) {
          }
          moveTo(687, 1182, 42, 1086, 300);
          break;
...
var appinfo = {};
appinfo.name = "顺丰速运xiaomi";
appinfo.bid = "com.sf.activity";
