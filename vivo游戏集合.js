...
//河马ip
function hmip() {
  var result = shell("ipclient 021180b9def94ec0bc182847d07cb079 1", true);
  // console.show();
  // log(result);
  if (result.code == 0) {
    toast("vpn 执行成功");
...
function main() {
  ...
  var regok_key = false;
  var timeLine = new Date().getTime();
  active(appinfo.bid, 4)
  while (new Date().getTime() - timeLine < 5 * 60 * 1000) {
    xianyang++;
    log(timeLine - new Date().getTime());
    if (jsclick("desc", "注册vivo帐号") || jsclick("text", "注册vivo帐号")) {
      return true
    }else if (text("立即下载").findOne(200) ){className("android.widget.Button").text("立即下载").findOne(200)
      return false
    }else{
       if (className("android.widget.Button").text("同意继续").findOne(200) ){
         className("android.widget.Button").text("同意继续").findOne(200).click()
        }else if (text("同意").findOne(200) ){
          text("同意").findOne(200).click()
        }else if (text("允许").findOne(200) ){
           text("允许").findOne(200).click()
        }
  }
  Tips()
  sleep(1000);
...
var appinfo = {};
appinfo.name = "剑玲珑vivo";
appinfo.bid = "com.sy4399.jllfx.vivo";
